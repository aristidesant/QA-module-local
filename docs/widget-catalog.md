...
- executes KPI widgets through the metric engine
...
- LINE/BAR/PIE/TABLE/FUNNEL widgets return `UNSUPPORTED` for now
...
```json
{
	...
	"widgets": [
		...
		},
		{
			...
			"status": "UNSUPPORTED",
			"result": null,
			"message": "Widget type BAR_CHART is not supported yet"
		}
	]
}
```
...
1. support `grouped` execution in the metric engine
2. support `time_series` execution
3. update dashboard render logic so `BAR_CHART`, `PIE_CHART`, `LINE_CHART`, and `TABLE` can resolve through the same engine
