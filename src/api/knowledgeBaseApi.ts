...
const knowledgeBaseApi = (_authHeader?: Record<string, string>) => {
  return {
    ...
    createKnowledgeBase: async (data: CreateKnowledgeBaseParams) => {
      ...
      if (typeof data.name !== "undefined") formData.append("name", data.name as string);
      if (typeof data.description !== "undefined") formData.append("description", data.description as string);
      if (typeof data.type !== "undefined") formData.append("type", data.type as string);
      if (typeof data.sourceUrl !== "undefined") formData.append("sourceUrl", data.sourceUrl as string);
      if (typeof data.textContent !== "undefined") formData.append("textContent", data.textContent as string);
      if (data.file) formData.append("file", data.file as any);

      const response = await axios.post(`${DEFAULT_API_URL}/knowledge-bases`, formData, {
        headers: {
    ...
    /**
     * Retrieve knowledge bases with optional filters.
     * Supported query params: agentId, type, status, search
     */
    ...
    getKnowledgeBase: async (id: number) => {
      const response = await axios.get(`${DEFAULT_API_URL}/knowledge-bases/${id}`, {
        headers: {
          ...(_authHeader || {}),
        },
      });
     ...
     */
    updateKnowledgeBase: async (id: number, data: Partial<CreateKnowledgeBaseParams>) => {
      const response = await axios.patch(`${DEFAULT_API_URL}/knowledge-bases/${id}`, data, {
        headers: {
          ...(_authHeader || {}),
        },
      });
    ...
    deleteKnowledgeBase: async (id: number) => {
      const response = await axios.delete(`${DEFAULT_API_URL}/knowledge-bases/${id}`, {
        headers: {
          ...(_authHeader || {}),
        },
      });
    ...
    retryKnowledgeBase: async (id: number) => {
      const response = await axios.post(`${DEFAULT_API_URL}/knowledge-bases/${id}/retry`, {}, {
        headers: {
          ...(_authHeader || {}),
        },
      });
