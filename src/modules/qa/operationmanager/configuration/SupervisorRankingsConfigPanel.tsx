...
interface SupervisorRankingsConfigPanelProps {
...
export const SupervisorRankingsConfigPanel: React.FC<SupervisorRankingsConfigPanelProps> = ({
	rankings = [],
	onSaveRanking,
	onDeleteRanking,
}) => {
	...
	return (
		<Stack gap="md">
			<Alert icon={<IconAlertCircle size={16} />} title="Supervisor Performance Rankings" color="blue">
				Define platform-wide supervisor performance rankings. Configure timeframe, metrics, and display settings.
			</Alert>
			...
			<Card p="lg" radius="md" withBorder>
				<Stack gap="md">
					<Group justify="space-between" align="flex-start">
						<div>
							<Text fw={600} size="md">
								Supervisor Rankings Configuration
							</Text>
							<Text size="xs" c="dimmed">
								Define and manage supervisor performance rankings across the platform
							</Text>
						</div>
			...
			<Modal opened={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Supervisor Ranking' : 'Create New Supervisor Ranking'} size="lg">
				<Stack gap="md">
					<TextInput label="Ranking Name" placeholder="e.g., September Supervisor Performance" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.currentTarget.value })} required />

					<Textarea label="Description" placeholder="Optional description of this ranking" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.currentTarget.value })} rows={3} />
					...
					<MultiSelect
						...
						placeholder="Select metrics to rank supervisors by"
