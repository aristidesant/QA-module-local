...
import type { FormValues as ReportValueFormValues } from './ReportValueFormModal';
import {
	ORIGIN_TYPE_COLORS,
	DATA_TYPE_COLORS,
	normalizeColumns,
...
export const useReportValueDraft = (campaignId: number | null) => {
	...
	const { data: columns = [], isLoading } =
		useGetReportColumns(resolvedCampaignId);
	const createMutation = useCreateReportValue(resolvedCampaignId, {
		invalidateOnSuccess: false,
	});
	const duplicateMutation = useDuplicateReportValue(resolvedCampaignId, {
		invalidateOnSuccess: false,
	});
	const bulkUpdateMutation = useBulkUpdateReportValues(resolvedCampaignId, {
		invalidateOnSuccess: false,
	});
	const deleteMutation = useDeleteReportValue(resolvedCampaignId, {
		invalidateOnSuccess: false,
	});
	...
	const handleDuplicateSubmit = useCallback(
		...
		[draftColumns, duplicateMutation, queryClient, resolvedCampaignId, t]
	);
